...
class Linear:
    ...
    def backward(self, dl_dy):
        ...
        self.db = np.sum(dl_dy, axis=0) #берутся только все первые элементы столбца и находится mean-значение среди них (просуммировать все и поделить на их кол-во)
                                        #при axix=1 брались бы все 1 элементы строки
        return self.dX
